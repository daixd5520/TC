...
class BiomedicalCoTGenerator:
    def __init__(self):
        # Biomedical类别映射 - 转换为Cxx格式
        ...
        self.api_configs = [
            {
                "name": "API_1",
                "api_key": "sk-urlgjtckkxgjnixqokwadjoalqbccsnxhpgscfhiqzymjcem",
                "base_url": "https://api.siliconflow.cn/v1",
                "model": "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B"
            },
            {
                "name": "API_2",
                "api_key": "sk-gkzgddprrvubatufpezpmnilvmculgppxnwpxzrhyjoaqxac",
                "base_url": "https://api.siliconflow.cn/v1",
                "model": "deepseek-ai/DeepSeek-R1-0528-Qwen3-8B"
            },
            {
                "name": "API_3",
                "api_key": "sk-ijzsytattqhmjdyijwufmhlysdpoedgqkmseyizamkctueet",
                "base_url": "https://api.siliconflow.cn/v1",
                "model": "deepseek-ai/DeepSeek-R1-0528-Qwen3-8B"
            },
            {
                "name": "API_4",
                "api_key": "sk-qbybfjwumzmytywaumzsbabbzchedsxzrjtgypvgkcpbleuo",
                "base_url": "https://api.siliconflow.cn/v1",
                "model": "deepseek-ai/DeepSeek-R1-0528-Qwen3-8B"
            },
            {
                "name": "API_5",
                "api_key": "sk-jfinkqinrsvzbtezbwteqcffwiaxpetfbmozzpyrqlkbpqkf",
                "base_url": "https://api.siliconflow.cn/v1",
                "model": "deepseek-ai/DeepSeek-R1-0528-Qwen3-8B"
            }
        ]
    ...
    def _build_system_prompt(self):
        """构建系统提示词 - 针对生物医学文本分类任务"""
        prompt = (
            "You are a biomedical topic classification expert. Given a medical text and its correct classification label, "
            "your task is to provide a detailed, step-by-step reasoning process that logically leads to the correct classification. "
            "Your explanation should analyze the medical content, biological terms, and key topics mentioned in the text.\n\n"
    ...
    def _build_user_prompt(self, text, true_label):
        """构建用户提示词"""
        return (
            "Please provide a detailed, step-by-step reasoning process for the following biomedical text classification task:\n\n"
            f"Text: {text}\n"
    ...
    def process_batch_parallel(self, batch_data, batch_id):
        ...
        # 过滤有效数据 - Biomedical使用text和label字段
        valid_batch_data = []
        for i, item in enumerate(batch_data):
            if (item.get('text') and item.get('text').strip() and 
                ...
                if label_num.isdigit():
                    ...
                    logger.warning(f"样本 {batch_id}_{i} 标签格式无效: {label_num}")
                    failed_count += 1
            else:
                ...
                for future in as_completed(future_to_item):
                    ...
                    try:
                        ...
                        if cot_response:
                            cot_item = {
                                "instruction": (
                                    "You are a biomedical topic classification expert. Carefully analyze the given medical text, "
...
def main():
    """主函数"""
    print("=" * 60)
    print("Biomedical数据集CoT数据生成")
    print("=" * 60)
    
    # 文件路径配置
    input_file = "/mnt/data1/TC/TextClassDemo/data/Biomedical/Biomedical_Train.json"
    output_file = "/mnt/data1/TC/TextClassDemo/data/Biomedical/Biomedical_Train_cot.json"
    checkpoint_file = "/mnt/data1/TC/TextClassDemo/data/Biomedical/Biomedical_Train_cot_checkpoint.json"
    ...
    # 创建生成器
    logger.info("初始化生成器...")
    generator = BiomedicalCoTGenerator()
